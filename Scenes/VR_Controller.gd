...
func _physics_process_directional_movement(delta):
	...
	# this fixes a bug about moving right if holding either grab because 'joystick' is how down the grab button is
#	var movement_vector = (trackpad_vector + joystick_vector).normalized()
	var movement_vector = trackpad_vector
	
	# Calculate the amount of movement the player will take on the Z (forward) axis and assign it to movement_forward.
	var movement_forward = forward_direction * movement_vector.x * delta * current_speed
	# Calculate the amount of movement the player will take on the X (right) axis and assign it to movement_forward.
	var movement_right = right_direction * movement_vector.y * delta * current_speed
	...
	if (movement_right.length() > 0 or movement_forward.length() > 0):
		...
		#directional_movement = true # removes vignette
...
func button_pressed(button_index):
	...
	# If the grab button is pressed...
		...
		# Call the _on_button_pressed_sprint function.
		_on_button_pressed_sprint()
...
# Pressing down the the analog stick to sprint
func _on_button_pressed_sprint():
	if current_speed < 4.5:
		current_speed = MOVEMENT_SPEED * 3
...
func _on_button_released_sprint():
